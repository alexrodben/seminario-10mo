<div class="card">
            ...
            <tbody>
                @forelse ($quotations as $quotation)
                    <tr>
                        ...
                        <td class="align-middle text-center">
                            {{ Number::currency($quotation->total_amount, 'QTZ') }}
                        </td>
                        <td class="align-middle text-center">
                        ...
                        <td class="align-middle text-center">
                            ...
                            @if ($quotation->status === \App\Enums\QuotationStatus::PENDING)
                                ...
                                <x-button.complete class="btn-icon"
                                    ...
                                    onclick="return confirm('¿Está seguro de completar la cotización No. {{ $quotation->reference }}?')" />
                                <x-button.delete class="btn-icon" route="{{ route('quotations.destroy', $quotation) }}"
                                    onclick="return confirm('¿Está seguro de cancelar la cotización No. {{ $quotation->reference }}?')" />
                            @endif
                        </td>
                    </tr>
                @empty
    ...
    <div class="card-footer d-flex align-items-center">
        <p class="m-0 text-secondary">
            Mostrando <span>{{ $quotations->firstItem() }}</span> de <span>{{ $quotations->lastItem() }}</span>
            de <span>{{ $quotations->total() }}</span> registros
        </p>
